#!/bin/bash
#
#########################################
# teamviewer.sh 下载TeamViewer
#########################################
LOG_FILE="/data/script/teamviewer.log"
BASE_DIR="/data/mirrors/application/teamViewer"
CMD="curl -Ls -o"

OS=("windows" "macos")

FILENAMES=()

# 日志输出
echo_log_info() {
  echo -e "$(date +'%F %T') - [info] $* " | tee -a $LOG_FILE
}
echo_log_err() {
  echo -e "$(date +'%F %T') - [error] $* " | tee -a $LOG_FILE
  echo >> $LOG_FILE
  exit 1
}

# 判断目录是否存在
#[ ! -d "$BASE_DIR" ] && echo_log_err "$BASE_DIR 目录不存在"

if [ ! -d "$BASE_DIR" ]; then
  echo_log_info "$BASE_DIR 目录不存在，正在创建..."
  mkdir -p "$BASE_DIR" || echo_log_err "无法创建目录 $BASE_DIR"
  echo_log_info "$BASE_DIR 目录创建成功"
fi

for i in "${OS[@]}"; do
  # 获取实际下载地址
  URL="https://www.teamviewer.cn/cn/download/${i}/"
  DOWNLOAD_URLS=$(curl -Ls $URL | grep -oP "<a class=\"cmp-button\" href=\"\K[^\"]+" | egrep "x64.exe|dmg")
  [ $? -ne 0 ] && echo_log_err "获取下载地址失败"

  # 获取下载版本
  DOWNLOAD_VER=$(curl -Ls $URL | grep -oP "<span data-dl-version-label>\K[0-9]+\.[0-9]+\.[0-9]+")
  [ $? -ne 0 ] && echo_log_err "获取网页下载版本失败"

  for DOWNLOAD_URL in $DOWNLOAD_URLS; do
    # 获取文件名
    NAME=$(basename "$DOWNLOAD_URL")
    FILENAME="${NAME[i]%.*}_$DOWNLOAD_VER.${NAME[i]#*.}"
    FILENAMES+=("$FILENAME")

    # 设置下载目录
    if [[ "${i}" == "windows" ]]; then
      DOWNLOAD_DIR="$BASE_DIR/win"
    else
      DOWNLOAD_DIR="$BASE_DIR/mac"
    fi

    LOCALFILE="$DOWNLOAD_DIR/$FILENAME"

    if [ ! -f "$LOCALFILE" ]; then
      echo_log_info "开始下载 $DOWNLOAD_URL"
      $CMD "$LOCALFILE" "$DOWNLOAD_URL" || echo_log_err "下载失败：$DOWNLOAD_URL"
    fi
  done
done

if [ -n "$(find "$BASE_DIR" -type f -name "TeamViewer*" $(printf "! -name %s " "${FILENAMES[@]}"))" ]; then
  echo_log_info "清理旧版本文件"
  [ ${#FILENAMES[@]} -ne 0 ] && find "$BASE_DIR" -type f -name "TeamViewer*" $(printf "! -name %s " "${FILENAMES[@]}") -exec rm -rf {} +
  echo >> $LOG_FILE
fi
