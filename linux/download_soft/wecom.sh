...
# wecom.sh 下载企业微信
#########################################
LOG_FILE="/data/script/wecom.log"
BASE_DIR="/data/mirrors/application/wecom"
CMD="curl -Ls -o"

OS=("win" "mac_arm64" "mac")
...
for i in "${OS[@]}"; do
  # 获取实际下载地址
  URL="https://work.weixin.qq.com/wework_admin/commdownload?platform=${i}&from=wwindex"
...
if [ -n "$(find "$BASE_DIR" -type f -name "WeCom*" $(printf "! -name %s " "${FILENAMES[@]}"))" ]; then
  echo_log_info "清理旧版本文件"
  [ ${#FILENAMES[@]} -ne 0 ] && find "$BASE_DIR" -type f -name "WeCom*" $(printf "! -name %s " "${FILENAMES[@]}") -exec rm -rf {} +
  echo >> $LOG_FILE
fi
