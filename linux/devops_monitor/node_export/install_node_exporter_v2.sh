...
# 6809dd0b3ec45fd6e992c19071d6b5253aed3ead7bf0686885a51d85c6643c66  node_exporter-1.8.2.linux-amd64.tar.gz
#
#
#
...
#https://github.com/prometheus/node_exporter/releases/download/v1.8.1/sha256sums.txt

# 版本校验码数据库
declare -A CHECKSUM_MAP=(
    ["1.8.1"]="fbadb376afa7c883f87f70795700a8a200f7fd45412532cc1938a24d41078011"
    ["1.8.2"]="6809dd0b3ec45fd6e992c19071d6b5253aed3ead7bf0686885a51d85c6643c66"
)

get_expected_checksum() {
    local version=$1
    local checksum=${CHECKSUM_MAP[$version]}
    
    if [ -z "$checksum" ]; then
        echo_log_error "未找到版本 $version 的校验码，请手动添加"
        return 1
    fi
...
download_package() {
    ...
    # 确保版本号正确传递到 check_version_exists
    if [[ -z "$version" ]]; then
    ...
    expected_checksum=$(get_expected_checksum "$version") || return $?
