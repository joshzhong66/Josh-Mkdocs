...
install_clash_ui() {
    if ! which docker >/dev/null 2>&1; then
        echo_log_error "Docker is not installed"
        if docker ps | grep yacd >dev/null 2>&1; then
            [ $? -eq 0 ] && echo_log_error "Clash UI Already installed, no need to reinstall."
        fi
    fi
